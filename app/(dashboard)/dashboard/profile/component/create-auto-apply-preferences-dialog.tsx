...
const autoApplySchema = z.object({
  ...
  max_daily_applications: z.coerce.number().optional(),
...
export function CreateAutoApplyPreferencesDialog() {
  const { isCreateAutoApplyPreferencesDialogOpen, setCreateAutoApplyPreferencesDialogOpen } = useProfileStore()
  const { post } = useApi()
  const { refreshPreferences } = useAutoApplyPreferences()
  ...
  const onSubmit: SubmitHandler<AutoApplyFormValues> = async (values) => {
    ...
    try {
      const payload = {
        ...
        max_daily_applications: values.max_daily_applications || 0,
      }

      const { error } = await post("applicants/auto-apply/preferences", payload)
      ...
      setCreateAutoApplyPreferencesDialogOpen(false)
      form.reset()
    } catch (err) {
  ...
  return (
    <Dialog open={isCreateAutoApplyPreferencesDialogOpen} onOpenChange={setCreateAutoApplyPreferencesDialogOpen}>
                          ...
                          {WORK_MODES.map((mode) => {
                            ...
                            return (
                              <Badge
                                ...
                                className={`cursor-pointer rounded-full px-4 py-2 text-xs font-normal ${
                                  isSelected
            ...
            <div className="border-t border-gray-100 bg-gray-50/50 px-6 py-4 dark:border-zinc-800 dark:bg-zinc-900/50">
              <div className="flex items-center justify-end gap-3">
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => setCreateAutoApplyPreferencesDialogOpen(false)}
